...
class RealPipelineTester:
    ...
    def __init__(self):
        ...
        logger.info(f"🎯 开始真实测试会话: {self.test_session_id}")
    ...
    def test_real_asr(self, audio_file):
        """测试真实ASR识别（使用真实阿里云API）"""
        logger.info("🤖 真实ASR识别测试")
        ...
        try:
            ...
            # 真实ASR识别
            recognition_result = asr_service.recognize_from_file(audio_file)
    ...
    def test_real_llm(self, asr_text):
        ...
        try:
            ...
            if response and response.strip():
                ...
                self.log_real_result("真实LLM生成", False, "LLM响应为空")
                return None
    ...
    def test_complete_real_pipeline(self):
        """测试完整的真实流程"""
        logger.info("🚀 开始完整真实ASR→LLM→TTS流程测试")

        # 清理之前的测试文件
        self.cleanup_old_files()

        print("\n" + "="*60)
        print("🎯 XLeRobot 真实流程测试")
    ...
    def generate_real_report(self):
        ...
        # 保存报告到文件
        report_file = f"real_pipeline_report_{self.test_session_id}.md"
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("# XLeRobot 真实流程测试报告\n\n")
